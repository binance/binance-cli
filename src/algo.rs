...
fn get_client(profile: Option<&str>, is_signed: bool) -> Result<RestApi, Error> {
    unsafe {
        env::set_var(
            ...
            build_user_agent("algo"),
        );
    }

    let config_rest_api = get_configuration_rest_api(profile, "algo").unwrap();
            ...
            None => {
                if args.interactive {
                    if args.algo_id.is_none() {
                        let algo_id: i64 = Input::new()
                            .with_prompt("Please enter the algo_id name")
                            .interact_text()?;
                ...
                if args.interactive {
                    ...
                    if args.duration.is_none() {
                        let duration: i64 = Input::new()
                            .with_prompt("Please enter the duration name")
                            .interact_text()?;
