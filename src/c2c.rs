...
fn get_client(profile: Option<&str>, is_signed: bool) -> Result<RestApi, Error> {
    unsafe {
        env::set_var("BINANCE_CONNECTOR_RUST_USER_AGENT", build_user_agent("c2c"));
    }

    let config_rest_api = get_configuration_rest_api(profile, "c2c").unwrap();
