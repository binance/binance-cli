...
alphaCommands.push({
    ...
    describe:
        decodeSelectedEntities(`Retrieves compressed, aggregated historical trades for a specific symbol. Useful for recent trade history.
...
alphaCommands.push({
    ...
    describe:
        decodeSelectedEntities(`Fetches general exchange information, such as supported symbols, rate limits, and server time.
...
alphaCommands.push({
    command: 'klines',
    describe:
        decodeSelectedEntities(`Fetches Kline/candlestick bars for a symbol, which include open/high/low/close prices and volume over intervals. Useful for charting and analysis.
...
alphaCommands.push({
    command: 'ticker',
    describe:
        decodeSelectedEntities(`Gets the 24-hour rolling window price change statistics for a symbol, including volume and price changes.
...
alphaCommands.push({
    command: 'token-list',
    describe:
        decodeSelectedEntities(`Retrieves a list of all available ALPHA tokens, including their IDs and symbols. Use this to find the token ID for constructing symbols in other endpoints.
