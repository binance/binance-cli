...
const getClient = () => {
    ...
    let basePath = DERIVATIVES_TRADING_USDS_FUTURES_WS_STREAMS_PROD_URL;

    const configurationRestAPI = getConfigurationRestAPI(parsedArgs?.profile, 'futures-usds');

    if (process.env.BINANCE_FUTURES_USDS_WS_STREAMS_BASE_PATH) {
        basePath = process.env.BINANCE_FUTURES_USDS_WS_STREAMS_BASE_PATH;
        ...
        switch (configurationRestAPI['env']) {
            case 'demo':
            case 'testnet':
                basePath = DERIVATIVES_TRADING_USDS_FUTURES_WS_STREAMS_TESTNET_URL;
                break;
        }
    }
    ...
    return {
        client: new DerivativesTradingUsdsFutures({ configurationWebsocketStreams }),
    };
};
...
derivativesTradingUsdsFuturesCommands.push({
    command: 'composite-index-symbol-information-streams',
    describe: decodeSelectedEntities(
...
Update Speed: 1000ms`,
    ...
    handler: async (options: any) => {
        ...
        try {
            const { client } = getClient();
            const connection = await client.websocketStreams.connect();
            const stream = connection.compositeIndexSymbolInformationStreams(options);
            const limit = options?.['stream-limit'];
            const startTime = Date.now();
            const durationMs = options?.['stream-duration'];
            let eventCount = 0;
...
derivativesTradingUsdsFuturesCommands.push({
    command: 'mark-price-stream-for-all-market',
    describe: decodeSelectedEntities(
...
Update Speed: 3000ms or 1000ms`,
    ...
    handler: async (options: any) => {
        ...
        try {
            const { client } = getClient();
            const connection = await client.websocketStreams.connect();
            const stream = connection.markPriceStreamForAllMarket(options);
            const limit = options?.['stream-limit'];
            const startTime = Date.now();
            const durationMs = options?.['stream-duration'];
            let eventCount = 0;
...
Update Speed: 1s`,
    ...
    handler: async (options: any) => {
        ...
        try {
            const { client } = getClient();
            const connection = await client.websocketStreams.connect();
            const stream = connection.multiAssetsModeAssetIndex(options);
            const limit = options?.['stream-limit'];
            const startTime = Date.now();
            const durationMs = options?.['stream-duration'];
            let eventCount = 0;
...
derivativesTradingUsdsFuturesCommands.push({
    command: 'trading-session-stream',
    describe: decodeSelectedEntities(
        `Trading session information for the underlying assets of TradFi Perpetual contracts—covering the U.S. equity market, Korean equity market, and the commodity market—is updated every second. Trading session information for different underlying markets is pushed in separate messages. Session types for the U.S. equity market include &quot;PRE_MARKET&quot;, &quot;REGULAR&quot;, &quot;AFTER_MARKET&quot;, &quot;OVERNIGHT&quot;, and &quot;NO_TRADING&quot;. Session types for the commodity market include &quot;REGULAR&quot; and &quot;NO_TRADING&quot;. Session types for the Korean equity market include &quot;REGULAR&quot; and &quot;NO_TRADING&quot;.
...
Update Speed: 1s`,
    ...
    handler: async (options: any) => {
        ...
        try {
            const { client } = getClient();
            const connection = await client.websocketStreams.connect();
            const stream = connection.tradingSessionStream(options);
            const limit = options?.['stream-limit'];
            const startTime = Date.now();
            const durationMs = options?.['stream-duration'];
            let eventCount = 0;
...
derivativesTradingUsdsFuturesCommands.push({
    command: 'rpi-diff-book-depth-streams',
    describe: decodeSelectedEntities(
...
Update Speed: 500ms`,
    ...
    handler: async (options: any) => {
        ...
        try {
            const { client } = getClient();
            const connection = await client.websocketStreams.connect();
            const stream = connection.rpiDiffBookDepthStreams(options);
            const limit = options?.['stream-limit'];
            const startTime = Date.now();
            const durationMs = options?.['stream-duration'];
            let eventCount = 0;
...
export default {
    command: 'futures-usds-streams',
    description: 'Binance Derivatives Trading USDS Futures WebSocket Market Streams',
    builder: (yargs: any) => {
        yargs.usage('Usage: binance-cli futures-usds-streams <command> [options]').options({
            pretty: {
                type: 'boolean',
