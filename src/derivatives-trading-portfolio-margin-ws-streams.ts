...
const getClient = () => {
    ...
    let basePath = DERIVATIVES_TRADING_PORTFOLIO_MARGIN_WS_STREAMS_PROD_URL;
    ...
    if (process.env.BINANCE_DERIVATIVES_PORTFOLIO_MARGIN_WS_STREAMS_BASE_PATH) {
        basePath = process.env.BINANCE_DERIVATIVES_PORTFOLIO_MARGIN_WS_STREAMS_BASE_PATH;
    ...
    return {
        client: new DerivativesTradingPortfolioMargin({ configurationWebsocketStreams }),
    };
};
...
export default {
    command: 'derivatives-portfolio-margin-streams',
    description: 'Binance Derivatives Trading Portfolio Margin WebSocket Market Streams',
    builder: (yargs: any) => {
        yargs
            .usage('Usage: binance-cli derivatives-portfolio-margin-streams <command> [options]')
            .options({
                pretty: {
                    type: 'boolean',
