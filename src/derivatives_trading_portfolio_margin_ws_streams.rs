...
use binance_sdk::derivatives_trading_portfolio_margin::DerivativesTradingPortfolioMarginWsStreams;
use binance_sdk::derivatives_trading_portfolio_margin::websocket_streams::WebsocketStreamsHandle;
use clap::Subcommand;
use dialoguer::Input;
use std::env;
...
fn get_client(profile: Option<&str>) -> Result<WebsocketStreamsHandle, Error> {
    ...
    let base_path = client_config
        ...
        .unwrap_or(match api_env.as_str() {
            "testnet" | "demo" => {
                DERIVATIVES_TRADING_PORTFOLIO_MARGIN_WS_STREAMS_TESTNET_URL.to_string()
            }
            "prod" => DERIVATIVES_TRADING_PORTFOLIO_MARGIN_WS_STREAMS_PROD_URL.to_string(),
            _ => {
    ...
    Ok(DerivativesTradingPortfolioMarginWsStreams::from_config(
        ws_config,
    ))
}

#[derive(Subcommand)]
pub enum DerivativesTradingPortfolioMarginWebsocketStreamsCommands {
    #[command(
...
pub async fn handle_derivatives_trading_portfolio_margin_ws_streams_command(
    command: DerivativesTradingPortfolioMarginWebsocketStreamsCommands,
) -> anyhow::Result<()> {
    match command {
        DerivativesTradingPortfolioMarginWebsocketStreamsCommands::UserData(args) => {
